...
<style>
    ...
    .table img {
        ...
        border-radius: 5px; /* Membuat gambar lebih rapi */
    }
</style>
    ...
    <div class="card mt-4">
        <div class="card-header d-flex justify-content-between align-items-center">
            ...
            @if (Auth::user()->role == 'admin')
                <a href="#" class="btn btn-sm btn-primary ms-auto" data-bs-toggle="modal" data-bs-target="#addProdukModal">
                    Tambah Produk
                </a>
            @endif
        </div>
                    ...
                    <tbody>
                        @foreach ($produk as $pro)
                            <tr>
                                ...
                                @if (Auth::user()->role == 'admin')
                                <td>
                                    <a href="#" class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#editProdukModal{{ $pro->id }}">
                                        Edit
                                    </a>
                                    <a href="#" class="btn btn-sm btn-warning" data-bs-toggle="modal" data-bs-target="#editStockModal{{ $pro->id }}">
                                        Edit Stok
                                    </a>
                                    <button type="button" class="btn btn-sm btn-danger delete-btn" data-id="{{ $pro->id }}">
                                        Hapus
                                    </button>
                                    <form id="delete-form-{{ $pro->id }}" action="{{ route('produk.destroy', $pro->id) }}" method="POST" class="d-none">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                </td>
