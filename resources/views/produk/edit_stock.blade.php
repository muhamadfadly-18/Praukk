<div class="modal fade" id="editStockModal{{ $pro->id }}" tabindex="-1"
    aria-labelledby="editStockModalLabel{{ $pro->id }}" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('produk.updateStock', $pro->id) }}" method="POST"
                    onsubmit="return confirmUpdate(event, this)">
                    @csrf
                    @method('PATCH')
                    <div class="mb-3">
                        <label for="stock" class="form-label">Stok</label>
                        <input type="text" id="editStock{{ $pro->id }}" name="stock" class="form-control"
                            value="{{ number_format($pro->stock, 0, ',', '.') }}" required>
                    </div>
