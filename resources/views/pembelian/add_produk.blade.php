...
@section('content')
            ...
            @foreach ($produks as $produk)
                <div class="col-md-4">
                    <div class="card text-center mb-4 mt-4">
                        <div class="card-body">
                            <img src="{{ asset($produk->gambar) }}" class="card-img-top mx-auto d-block"
                                alt="{{ $produk->nama }}" style="width: 150px; height: 210px; object-fit: cover;">
                            <h5 class="card-title mt-2">{{ $produk->name }}</h5>
                            <p class="card-text">Stock: <b id="stock-{{ $produk->id }}">{{ $produk->stock }}</b></p>
                            <p class="card-text text-success fw-bold">Harga: Rp
                                {{ number_format($produk->harga_beli, 0, ',', '.') }}</p>
                            <p class="card-text text-danger fw-bold">Subtotal: Rp <span
                                    id="subtotal-{{ $produk->id }}">0</span></p>
                            <div class="input-group">
                                <button type="button" class="btn btn-outline-secondary"
                                    onclick="kurangiJumlah({{ $produk->id }}, {{ $produk->harga_beli }})">-</button>
                                <input type="number" class="form-control text-center" id="jumlah-{{ $produk->id }}"
                                    name="jumlah" value="0" min="0" max="{{ $produk->stock }}" readonly>
                                <button type="button" class="btn btn-outline-secondary"
                                    onclick="tambahJumlah({{ $produk->id }}, {{ $produk->stock }}, {{ $produk->harga_beli }})">+</button>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <form id="pesanForm" action="{{ url('/pembelian/simpan') }}" method="POST">
        ...
        <button type="submit" class="btn btn-primary fixed-bottom-btn" id="pesanBtn" onclick="submitPesanan()"
    ...
    <script>
        ...
        function submitPesanan() {
            ...
            document.querySelectorAll('input[name="jumlah"]').forEach(input => {
                ...
                let jumlah = parseInt(input.value);
                let harga = document.querySelector(`#subtotal-${id}`).innerText.replace(/\./g,
                ''); // Ambil harga asli
