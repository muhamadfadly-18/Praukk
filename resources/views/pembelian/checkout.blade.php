                ...
                <div class="col-md-6">
                    <div class="card shadow-sm p-4">
                        ...
                        <div class="d-flex justify-content-between">
                            ...
                            <h5 class="fw-bold">Rp
                                {{ number_format(collect($produks)->sum(fn($p) => $p->harga_beli * $p->jumlah), 0, ',', '.') }}
                            </h5>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card shadow-sm p-4">
                        ...
                        <form id="checkoutForm" method="POST" action="">
                            ...
                            <input type="hidden" name="total_harga"
                                value="{{ collect($produks)->sum(fn($p) => $p->harga_beli * $p->jumlah) }}">
                            ...
                            <small id="total_bayar_error" class="text-danger" style="display: none;">Total Bayar harus lebih
                                dari Total Harga</small>
            ...
            $(document).ready(function() {
                ...
                $('#checkoutForm').submit(function(e) {
                    ...
                    if (no_hp.length >= 10) {
                        ...
                        $.ajax({
                            ...
                            error: function(xhr) {
                                console.error('Gagal menyimpan detail pembelian:', xhr
                                .responseJSON);
                                alert('Gagal menyimpan detail pembelian: ' + xhr.responseJSON
                                    .message);
                            }
                        });
                    }
                ...
                $('#no_hpNumber').on('input', function() {
                    ...
                    if (!/^((08|62)[0-9]*)$/.test(no_hp)) {
                        $('#no_hpStatus').text('Nomor HP harus dimulai dengan 08 atau 62').addClass(
                            'text-danger');
                        $('#pesanBtn').prop('disabled', true);
